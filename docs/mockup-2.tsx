...
const SCENARIO = {
    ...
    // Logic: Advance to next protocol when Resistance drops below threshold.
    protocols: [
        {
            id: 'p1',
...
const ItemCard = ({ 
    ...
    return (
        <div 
            ...
            className={`
                relative w-24 h-36 rounded-xl flex flex-col p-2 transition-all cursor-pointer overflow-hidden
                ${selected 
                ...
                ${(disabled || lockedTurns) ? 'opacity-60 grayscale' : ''}
            `}
        >
            {/* Scan Mode Indicator overlay */}
            {isScanMode && selected && (
            ...
            <div className="mt-1 mb-auto flex flex-col">
                <div className="text-[10px] text-indigo-600 font-bold uppercase tracking-tight mb-0.5">{card.source}</div>
                <div className="text-[10px] font-bold leading-tight text-gray-900 line-clamp-2">{card.title}</div>
                <div className="text-[9px] text-gray-500 font-medium mt-1 leading-tight">{card.desc}</div>
            </div>
...
export const GameInterface = () => {
    ...
    // Engine State
    const [security, setSecurity] = useState(SCENARIO.startSecurity);
    const [turns, setTurns] = useState(SCENARIO.turns);
    const [risk, setRisk] = useState(2); // Start at manageable risk 
    ...
    const validatePayload = (cards: ReceiptCard[]): { valid: boolean; message?: string; matchName?: string } => {
        // Validation is STRICTLY based on the Active Protocol
    ...
    useEffect(() => {
        ...
        if (mode === 'SCAN') {
            setAuraMessage(scanSelection.length > 0 ? "Purging data? Fine. Costs 1 Turn + 2 Risk." : "Searching for a loophole?");
    ...
    const applyRisk = (delta: number, source: 'UPLOAD' | 'SCAN') => {
        ...
        if (newRisk >= 5) {
            ...
            if (cardToQuarantine) {
                ...
                // If the quarantined card is currently in upload slots, remove it
                setUploadSlots(prev => prev.filter(c => c.id !== cardToQuarantine!.id));
                setScanSelection(prev => prev.filter(id => id !== cardToQuarantine!.id));
                
                msg = `AUDIT TRIGGERED! Resistance +20. Quarantined: ${cardToQuarantine.title}`;
            } else {
                msg = `AUDIT TRIGGERED! Resistance +20. System Reboot.`;
            }
            mood = 'hostile';
        }
    ...
    const checkProgression = (currentSecurity: number) => {
        const total = SCENARIO.protocols.length;
        if (activeProtocolIndex >= total - 1) return; // Last protocol already active

        // Progression Threshold Logic
        // For 2 protocols: Advance at <= 50%
        let threshold = 50; 
    ...
    const handleUpload = () => {
        ...
        if (validation.valid) {
            ...
            damage = 0;
            riskDelta += 1; // Penalty for invalid proof
            msg = "Proof Rejected. 0 Damage. +1 Risk.";
            mood = 'annoyed';
        }

        // 2. Sketchy Check (Always applies)
        ...
        // 3. Apply Damage
        let newSecurity = Math.max(0, security - damage);
        setSecurity(newSecurity);

        // 4. Advance Turn
        advanceTurn();
        setUploadSlots([]); // Clear slots

        // 5. Apply Risk & Check Audit
        if (riskDelta > 0) {
            const auditResult = applyRisk(riskDelta, 'UPLOAD');
            if (auditResult.auditTriggered) {
                msg = auditResult.msg;
                mood = auditResult.mood;
                // Security might have healed in applyRisk, so we update UI, 
                // but checking progression should ideally happen on 'net' security. 
                // For simplicity, we check progression on the result of THIS action, 
                // knowing audit might undo some progress next tick or immediately.
            }
        }

        // 6. Check Protocol Progression
        const progressionMsg = checkProgression(newSecurity);
        if (progressionMsg) {
             msg = progressionMsg;
             mood = 'scrutinizing';
        }
    ...
    const handleScan = () => {
        if (turns <= 0 || risk >= 5 || scanSelection.length === 0 || scanSelection.length > 2) return;

        // 1. Cost: 1 Turn, +2 Risk
        advanceTurn();
        const auditResult = applyRisk(2, 'SCAN');

        // 2. Perform Swap (Deterministic)
        const rng = pseudoRandom(`${SCENARIO.id}-${rngSeed}-${scanCount}`);
                    ...
                    <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 w-full max-w-sm text-left shadow-sm">
                        ...
                        <div className="space-y-3">
                            {SCENARIO.protocols.map((p, idx) => (
                                <div key={p.id} className="flex gap-3 items-start opacity-100">
        ...
        <div className="flex flex-col h-full bg-gray-50 font-sans relative overflow-hidden">
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4 z-20 shadow-[0_4px_20px_rgba(0,0,0,0.03)] shrink-0 rounded-b-3xl">
                <div className="flex justify-between items-end mb-2">
                    ...
                    <span className="text-xs font-bold text-gray-900">{security}%</span>
                </div>
            ...
            <div className="bg-white border-t border-gray-200 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-30 shrink-0 pb-safe rounded-t-3xl transition-all">
                ...
                {/* SLOTS */}
                {mode === 'UPLOAD' && (
                    <div className="flex gap-4 justify-center items-center pt-2 pb-4 px-6">
                        {[0, 1].map(idx => {
                             // Visual disable for Slot B if SINGLE Protocol
                                ...
                                <div 
                                    ...
                                    className={`
                                        ...
                                        ${isLockedSlot ? 'bg-gray-100 border-gray-200 opacity-50 cursor-not-allowed border-dashed' : 
                                            uploadSlots[idx] 
                                            ? 'border-indigo-200 shadow-sm bg-indigo-50 border-solid' 
                                            : 'border-gray-200 hover:border-gray-300 bg-gray-50 border-dashed'}
                                    `}
                                >
                                    {uploadSlots[idx] ? (
                                        <ItemCard card={uploadSlots[idx]} disabled={false} mode={mode} />
                                    ) : (
                ...
                {mode === 'SCAN' && (
                     <div className="h-32 flex items-center justify-center pt-2 pb-4 animate-in slide-in-from-bottom-2">
                         <p className="text-xs text-gray-500 font-medium px-8 text-center">
                            ...
                            <span className="text-red-500 font-bold">Cost: 1 Turn + 2 Risk</span>
                         </p>
                     </div>
                )}
                ...
                <div className="px-6 mb-6">
                    {mode === 'UPLOAD' ? (
                        ...
                        >
                            ...
                            {uploadSlots.length > 0 && (
                                <span className="text-[10px] font-mono opacity-80">
                                    PROJECTED DAMAGE: {validation.valid ? basePower : '0 (BLOCKED)'}
                                </span>
                            )}
                        </button>
                    ) : (
                        <button 
                            onClick={handleScan}
                            disabled={turns <= 0 || scanSelection.length === 0 || scanSelection.length > 2}
                            className={`
                                ...
                                ${scanSelection.length > 0 && scanSelection.length <= 2
                                    ? 'bg-white border-2 border-red-500 text-red-500 hover:bg-red-50'
                                    : 'bg-gray-100 text-gray-400 cursor-not-allowed'}
                            `}
                        >
                            <span>SCAN {scanSelection.length > 0 ? `(${scanSelection.length} ITEMS)` : ''}</span>
                            <span className="text-[8px] font-bold uppercase">+2 RISK</span>
                        </button>
                    )}
                </div>
                        ...
                        {hand.filter(c => !uploadSlots.find(p => p.id === c.id)).map(card => (
                            <div key={card.id} className="shrink-0 snap-center">
                                <ItemCard 
                                    ...
                                    onClick={() => {
                                        if (mode === 'UPLOAD') {
                                            // Only allow adding if protocol supports more slots and card isn't quarantined
