...
interface Protocol {
    ...
    slots: 1 | 2;
    check: (cards: ReceiptCard[]) => { valid: boolean; reason: string; matchName?: string };
}
...
const AURA_QUIPS = {
    VALID_SINGLE: [
        "Sensor accepted. You're still annoying.",
        "Data verified. Don't get used to it.",
        "Fine. I'll unlock it slightly.",
        "Adequate proof. Proceed."
    ],
    VALID_CORROBORATED: [
        "Two systems agree. Unfortunate for me.",
        "Cute. Now try real proof.",
        "Corroboration verified. You're learning.",
        "Synchronized timestamps. How bureaucratic of you."
    ],
    INVALID: [
        "Inadmissible. Try reading the manual.",
        "This proves nothing.",
        "I can't accept garbage data.",
        "Is this a joke? Rejected."
    ],
    SCAN: [
        "Searching for a loophole?",
        "Purging data? Fine. It costs you.",
        "Scrambling the evidence won't save you.",
        "I'm watching your little shuffle."
    ]
};

const getRandomQuip = (category: keyof typeof AURA_QUIPS) => {
    const pool = AURA_QUIPS[category];
...
const SCENARIO = {
    id: '2026-01-30-FRIDGE-LOCK',
    targetName: 'SMART FRIDGE v4',
    story: {
        problem: "The fridge thinks you're 'night snacking' again.",
        reason: "Circadian Discipline Protocol.",
        goal: "Submit valid Proof Packets until Resistance hits 0."
    },
    turns: 8,
    startResistance: 100,
    ...
    protocols: [
        {
            ...
            shortInstruction: 'Submit 1 VERIFIED SENSOR.',
            why: "AURA trusts sensors more than your words.",
            type: 'SINGLE',
            slots: 1,
            check: (cards: ReceiptCard[]) => {
                if (cards.length !== 1) return { valid: false, reason: 'Empty Slot' };
                const c = cards[0];
                if (c.trust !== 'VERIFIED') return { valid: false, reason: 'Source must be VERIFIED' };
                if (!c.tags.includes('SENSOR')) return { valid: false, reason: 'Source is not a SENSOR' };
                return { valid: true, reason: 'Verified Sensor Data', matchName: `Verified Sensor (${c.source})` };
            }
        },
        {
            id: 'p2',
            label: 'Protocol B: Corroboration',
            shortInstruction: 'Submit 2 Independent Sources (TIME match).',
            why: "One source is a glitch. Two is a fact.",
            type: 'CORROBORATED',
            slots: 2,
            check: (cards: ReceiptCard[]) => {
                if (cards.length !== 2) return { valid: false, reason: 'Need 2 receipts' };
                const [c1, c2] = cards;
                
                // Check Tags
                const timeMatch = c1.tags.includes('TIME') && c2.tags.includes('TIME');
                if (!timeMatch) return { valid: false, reason: 'Timestamps do not match' };

                // Check Independence
                if (c1.source === c2.source) return { valid: false, reason: `Same Source (${c1.source})` };
                
                return { valid: true, reason: 'Time Match + Independent Sources', matchName: `Crosscheck (${c1.source} + ${c2.source})` };
            }
        }
    ] as Protocol[],
...
const ItemCard = ({ 
    ...
    const glowClass = !disabled && isEligible && !isTraySlot && !isScanMode
        ? 'ring-2 ring-teal-400 ring-offset-1 shadow-[0_0_10px_rgba(45,212,191,0.3)]' 
        : '';
        
    const mutedClass = !disabled && !isEligible && !isTraySlot && !isScanMode
        ? 'opacity-60 saturate-50 scale-95'
        : 'opacity-100';

    const selectionClass = selected 
        ? (isScanMode ? 'bg-red-50 border-2 border-red-400 ring-2 ring-red-100' : 'bg-indigo-50 border-2 border-indigo-500 shadow-md') 
...
export const GameInterface = () => {
    ...
    // Selection States
    const [proofTray, setProofTray] = useState<ReceiptCard[]>([]); 
    ...
    const [auraMessage, setAuraMessage] = useState("Awaiting input.");
    ...
    const validatePayload = (cards: ReceiptCard[]): { valid: boolean; reason: string; matchName?: string } => {
        if (cards.length === 0) return { valid: false, reason: 'Proof Tray Empty' };
        // Check slot count match first
        if (cards.length !== activeProtocol.slots) {
            return { valid: false, reason: `Requires exactly ${activeProtocol.slots} receipts` };
        }
        return activeProtocol.check(cards);
    };

    const validation = validatePayload(proofTray);
    const basePower = proofTray.reduce((sum, c) => sum + c.power, 0);
    
    // Helper to check if a card IN HAND is potentially useful for current protocol
    const checkEligibility = (card: ReceiptCard) => {
        if (activeProtocol.type === 'SINGLE') {
            const res = activeProtocol.check([card]);
            return res.valid;
        } 
        if (activeProtocol.type === 'CORROBORATED') {
            // Harder to check exact eligibility without a partner, 
            // but we can check if it has the basic tags required (TIME usually)
    ...
    useEffect(() => {
        ...
        if (mode === 'SCAN') {
            if (scanSelection.length > 0) {
                setAuraMessage(getRandomQuip('SCAN'));
                setAuraMood('scrutinizing');
            } else {
                setAuraMessage("Select cards to discard.");
                setAuraMood('calm');
            }
            return;
        }

        // Proof Mode Reactions
        if (proofTray.length === 0) {
            setAuraMessage("Enforce the protocol.");
            setAuraMood('calm');
        } else if (validation.valid) {
             setAuraMessage(validation.reason); // Show the mechanic reason first
             setAuraMood('success');
        } else if (proofTray.length > 0) {
            setAuraMessage(validation.reason); // Show error reason
            setAuraMood('annoyed');
        }
    }, [proofTray, mode, scanSelection, phase, validation.valid, validation.reason]);
    ...
    const applyRisk = (delta: number, source: 'UPLOAD' | 'SCAN') => {
        let newRisk = risk + delta;
        ...
        if (newRisk >= 5) {
            ...
            if (cardToQuarantine) {
                ...
                setProofTray(prev => prev.filter(c => c.id !== cardToQuarantine!.id));
                setScanSelection(prev => prev.filter(id => id !== cardToQuarantine!.id));
                msg = `AUDIT! Flagged: ${cardToQuarantine.title}`;
            } else {
                msg = `AUDIT! Resistance +20.`;
            }
            mood = 'hostile';
        }
    ...
    const handleUpload = async () => {
        if (turns <= 0 || proofTray.length === 0) return;
        
        const payload = [...proofTray];
        setLastSubmittedPayload(payload);

        // 1. Stamp Animation
        setSubmitAnim(validation.valid ? 'STAMP_VALID' : 'STAMP_INVALID');
        await new Promise(r => setTimeout(r, 600)); // Wait for stamp
        ...
        if (validation.valid) {
            damage = basePower;
            msg = getRandomQuip(activeProtocol.type === 'SINGLE' ? 'VALID_SINGLE' : 'VALID_CORROBORATED');
            mood = 'success';
        } else {
            damage = 0;
            riskDelta += 1;
            msg = getRandomQuip('INVALID');
            mood = 'annoyed';
        }
        ...
        // 5. Audit Check
        if (riskDelta > 0) {
            const auditResult = applyRisk(riskDelta, 'UPLOAD');
        ...
        // 6. Progression Check
        // Advance Protocol if resistance drops below 50%
        const totalProtocols = SCENARIO.protocols.length;
        if (activeProtocolIndex < totalProtocols - 1) {
             if (newResistance <= 50) {
                setActiveProtocolIndex(prev => prev + 1);
                setProofTray([]);
                msg = "Protocol Bypassed. Updating constraints...";
                mood = 'scrutinizing';
             }
        }
    ...
    const handleScan = () => {
        if (turns <= 0 || risk >= 5 || scanSelection.length === 0) return;

        advanceTurn();
        const auditResult = applyRisk(2, 'SCAN');
            ...
            <div className="flex flex-col h-full bg-white p-6 relative animate-in fade-in duration-500 overflow-y-auto">
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8">
                    <div className="relative">
                        ...
                        <AuraAvatar mood="scrutinizing" className="w-28 h-28 relative z-10" />
                    </div>
                    
                    <div className="space-y-4 max-w-xs mx-auto">
                        <div className="bg-gray-50 rounded-2xl p-6 border border-gray-200 shadow-sm text-left space-y-3">
                            <div>
                                <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tonight's Problem</div>
                                <h2 className="text-lg font-bold text-gray-900 leading-tight">{SCENARIO.story.problem}</h2>
                            </div>
                            <div>
                                <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">AURA's Reason</div>
                                <p className="text-sm text-indigo-600 font-medium font-mono">"{SCENARIO.story.reason}"</p>
                            </div>
                            <div className="pt-2 border-t border-gray-200">
                                <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Your Goal</div>
                                <p className="text-sm text-gray-700">{SCENARIO.story.goal}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <button 
                        onClick={startDraft}
                        className="w-full max-w-xs py-4 bg-gray-900 text-white font-bold rounded-2xl shadow-xl hover:scale-[1.02] active:scale-[0.95] transition-all flex items-center justify-center gap-2"
                    >
                        <span>Gather Receipts (Pick 6)</span>
                ...
                <div className="flex-1 flex flex-col justify-center items-center text-center space-y-8">
                    ...
                    <div>
                        <h1 className={`text-4xl font-black tracking-tighter mb-2 ${win ? 'text-teal-900' : 'text-red-900'}`}>
                            {win ? 'UNLOCKED' : 'LOCKDOWN'}
                        </h1>
                        <p className="text-sm font-medium text-gray-600 max-w-xs mx-auto">
                            {win 
                                ? "Protocol satisfied. You may eat your cheese." 
                                : "Too many deviations. The fridge stays shut."}
                        </p>
                    </div>
                    <button 
        ...
        <div className="flex flex-col h-full bg-gray-50 font-sans relative overflow-hidden">
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4 z-20 shadow-[0_4px_20px_rgba(0,0,0,0.03)] shrink-0 rounded-b-3xl">
                ...
                <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden mb-4 shadow-inner relative">
                    ...
                    ></div>
                    <div className="absolute top-0 bottom-0 w-0.5 bg-white/50 left-1/2" title="Protocol Shift"></div>
                </div>

                <div className="flex justify-between items-center">
                    <div className="flex flex-col gap-1">
                        <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Risk</span>
                        <div className="flex gap-1">
                            {[1,2,3,4,5].map(i => (
                                <div key={i} className={`
                                    w-3 h-3 rounded-full transition-colors duration-300
                                    ${i <= risk 
                                        ? (risk >= 5 ? 'bg-red-600 animate-ping' : risk >= 4 ? 'bg-red-500' : 'bg-orange-400') 
                                        : 'bg-gray-200'}
                                `} />
                            ))}
                        </div>
            ...
            <div className="flex-1 relative flex flex-col justify-start items-center p-4 min-h-0 overflow-y-auto">
                 ...
                 {/* CHALLENGE CARD (Protocol) */}
                 <div className="w-full mb-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
                     <div className="absolute top-0 left-0 w-1 h-full bg-teal-500"></div>
                     <div className="flex justify-between items-start mb-1">
                        <div className="text-[10px] font-bold text-teal-600 uppercase tracking-wider">Active Protocol</div>
                        <div className="text-[9px] font-mono text-gray-400">LVL {activeProtocolIndex + 1}</div>
                     </div>
                     <div className="text-sm font-bold text-gray-900 leading-tight mb-2">{activeProtocol.label}</div>
                     <div className="bg-gray-50 p-2 rounded border border-gray-100 mb-2">
                        <div className="text-xs font-medium text-gray-800 flex gap-2 items-center">
                            {activeProtocol.type === 'SINGLE' 
                                ? <span className="w-4 h-4 rounded border-2 border-gray-400 block" />
                                : <div className="flex -space-x-1"><span className="w-4 h-4 rounded border-2 border-gray-400 block bg-white" /><span className="w-4 h-4 rounded border-2 border-gray-400 block bg-white" /></div>
                            }
                            {activeProtocol.shortInstruction}
                        </div>
                     </div>
                     <p className="text-[10px] text-gray-500 italic">"Why: {activeProtocol.why}"</p>
                 </div>

                 <div className="flex items-center gap-4 w-full justify-center">
                     <AuraAvatar mood={auraMood} className="w-16 h-16 transition-all duration-500 shrink-0" />
                     <div className="bg-white px-4 py-3 rounded-xl shadow-sm border border-gray-200 flex-1 relative animate-in zoom-in duration-300">
                        <p className="text-xs font-medium text-gray-700 leading-relaxed">
                            "{auraMessage}"
                        </p>
                        <div className="absolute top-1/2 -left-1.5 -mt-1.5 w-3 h-3 bg-white border-b border-l border-gray-200 rotate-45"></div>
                     </div>
                 </div>
            </div>
            ...
            <div className="bg-white border-t border-gray-200 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-30 shrink-0 pb-safe rounded-t-3xl transition-all">
                ...
                <div className="flex justify-center -mt-5 mb-2">
                    <div className="bg-gray-200 p-1 rounded-full flex shadow-sm">
                        ...
                        <button 
                             onClick={() => { setMode('SCAN'); setProofTray([]); }}
                             className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all
                ...
                {/* PROOF TRAY (Interactive) */}
                {mode === 'UPLOAD' && (
                    <div className="relative pt-2 pb-4 px-6">
                        {/* Validation Text */}
                        <div className="text-center mb-2 h-4">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${validation.valid ? 'text-green-600' : 'text-gray-400'}`}>
                                {proofTray.length > 0 
                                    ? (validation.valid ? `VALID: ${validation.matchName}` : `ISSUE: ${validation.reason}`)
                                    : "Drag or Tap Receipts below"}
                            </span>
                        </div>
                        ...
                        <div className="flex gap-4 justify-center items-center relative">
                            ...
                            {activeProtocol.slots === 2 && proofTray.length === 2 && validation.valid && (
                            ...
                            {[0, 1].map(idx => {
                                 const isLockedSlot = (activeProtocol.slots === 1 && idx === 1);
                                 return (
                                    <div 
                                        key={idx}
                                        onClick={() => !isLockedSlot && proofTray[idx] && setProofTray(proofTray.filter(p => p.id !== proofTray[idx].id))}
                                        className={`
                                            w-24 h-36 rounded-xl border-2 flex items-center justify-center relative z-10 transition-all cursor-pointer 
                                            ${isLockedSlot ? 'bg-gray-50 border-gray-100 opacity-40 cursor-not-allowed' : 
                                                proofTray[idx] 
                                    ...
                                    >
                                        {proofTray[idx] ? (
                                            <ItemCard card={proofTray[idx]} disabled={false} mode={mode} isTraySlot={true} />
                                        ) : (
                            ...
                            {submitAnim !== 'IDLE' && (
                                <div className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none">
                                    <div className={`
                                        text-2xl font-black uppercase tracking-widest border-4 px-6 py-2 rotate-[-12deg] opacity-0 animate-[ping_0.5s_ease-out_forwards]
                ...
                {mode === 'SCAN' && (
                     <div className="h-44 flex flex-col items-center justify-center animate-in slide-in-from-bottom-2 px-6">
                         <div className="w-12 h-12 bg-red-50 rounded-full flex items-center justify-center text-red-500 mb-2">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" /><path d="M3 3v5h5" /><path d="M3 12a9 9 0 0 0 9 9 9.75 9.75 0 0 0 6.74-2.74L21 16" /><path d="M16 16h5v5" /></svg>
                         </div>
                         <p className="text-xs text-gray-500 font-medium text-center">
                            Select up to 2 receipts to discard & draw new ones.
                         </p>
                         <div className="mt-2 text-[10px] font-bold bg-red-100 text-red-600 px-2 py-1 rounded">Cost: 1 Turn + 2 Risk</div>
                     </div>
                )}
                ...
                <div className="px-6 mb-6">
                    {mode === 'UPLOAD' ? (
                        <button 
                            ...
                            disabled={turns <= 0 || proofTray.length === 0}
                            className={`
                        ...
                        <button 
                            onClick={handleScan}
                            disabled={turns <= 0 || scanSelection.length === 0}
                            className={`
                            ...
                            `}
                        >
                            <span>REFRESH {scanSelection.length > 0 ? `(${scanSelection.length})` : ''}</span>
                        </button>
                    )}
                </div>
                ...
                <div className="px-6 pb-6">
                     <div className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-2">Receipt Hand</div>
                     <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 snap-x">
                        {hand.filter(c => !proofTray.find(p => p.id === c.id)).map(card => {
                                    ...
                                    <ItemCard 
                                        ...
                                        onClick={() => {
                                            if (mode === 'UPLOAD') {
                                                ...
                                                if (proofTray.length < activeProtocol.slots) {
                                                    setProofTray([...proofTray, card]);
                                                }
                                            } else {
                                                if (quarantine?.id === card.id) return;
