...
#
# Purpose: Review a feature plan (no task files) for internal consistency,
...
inline task details are coherent, accurate, and implementable without surprises.
...
- Inline task details in the plan
...
- The plan is the source of truth for task status and task details.
...
- For each task: count AC/EC/ERR and confirm they are concrete.
...
- If task files exist, it's a Medium issue (ambiguous source of truth).
