...
function simulateAll(puzzle: Puzzle): PlaySequence[] {
  ...
  function dfs(
    ...
    if (state.resistance <= 0 || state.turnsRemaining <= 0 || state.scrutiny >= puzzle.scrutinyLimit) {
      results.push({
        turns: turnsSoFar,
...
function findNaivePath(puzzle: Puzzle): PlaySequence {
  ...
  for (const card of sorted) {
    if (state.turnsRemaining <= 0 || state.resistance <= 0 || state.scrutiny >= puzzle.scrutinyLimit) break;
...
const THE_LAST_SLICE_V25: Puzzle = {
  ...
  cards: [
    ...
    { id: 'microwave',    power: 2, tag: Tag.IDLE,   risk: 0, proves: ProofType.ALERTNESS, source: 'smart_microwave', flavor: 'Smart microwave' },
  ],
  counters: [
...
function runAnalysis(puzzle: Puzzle) {
  ...
  console.log('\n--- AMBIGUOUS FEEDBACK DEMO (naive path) ---');
  {
    ...
    let turn = 0;
    for (const card of sorted) {
      if (demoState.turnsRemaining <= 0 || demoState.resistance <= 0 || demoState.scrutiny >= puzzle.scrutinyLimit) break;
